...
class BookingApp:
    def __init__(self, master):
        ...
        tk.Label(master, text="ชื่อห้อง:").pack()
        self.room_entry = tk.Entry(master)
        self.room_entry.pack()

        tk.Button(master, text="เพิ่มห้อง", command=self.add_room).pack(pady=5)

        tk.Label(master, text="ชื่อผู้จอง:").pack()
        self.user_entry = tk.Entry(master)
        self.user_entry.pack()

        tk.Label(master, text="เวลาที่ต้องการจอง (ตัวเลขชั่วโมง เช่น 10):").pack()
        self.time_entry = tk.Entry(master)
        self.time_entry.pack()

        tk.Button(master, text="จองห้อง", command=self.book_room).pack(pady=5)
        tk.Button(master, text="ตรวจสอบห้องว่าง", command=self.check_room).pack(pady=5)
        tk.Button(master, text="ประมวลผลคิวรอ", command=self.process_queue).pack(pady=5)
        tk.Button(master, text="ดูสถานะทั้งหมด", command=self.show_status).pack(pady=5)

        self.output_box = tk.Text(master, height=15, width=70)
